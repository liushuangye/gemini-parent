    ...
    <div class="usernamage-wrap">
      <div class="search">
        <el-form :inline="true">
          ...
          <el-form-item label-width="1px">
            ...
            <el-button type="primary" size="mini" @click="showDialogVisible()">{{ $t('devopstools.createTask') }}</el-button>
          </el-form-item>
        </el-form>
      </div>
      <div class="line" />
      <div class="tabledata">
        ...
        >
          ...
          <el-table-column align="center" :label="$t('devopstools.taskName')" min-width="220">
            <template v-slot="scope">
              {{ scope.row.taskName }}
            </template>
          </el-table-column>
          <el-table-column align="center" :label="$t('devopstools.toolName')" min-width="220">
            <template v-slot="scope">
          ...
          <el-table-column align="center" :label="$t('devopstools.createTime')" min-width="200">
            <template v-slot="scope">
      ...
      <el-dialog
        ...
        :title="$t('devopstools.createTask')"
        width="30%"
      >
        <el-form label-width="25%">
          <el-form-item :label="$t('devopstools.taskName')">
            <el-input v-model="creatFrom.taskName" size="mini" style="width: 80%;" />
          </el-form-item>
          <el-form-item :label="$t('devopstools.toolName')">
            <el-select v-model="creatFrom.devopsId" filterable size="mini" placeholder="请选择" style="width: 80%;">
              <el-option
        ...
        <div slot="footer" style="text-align:right">
          <el-button type="primary" size="mini" @click="create()">{{ $t('common.btnConfirm') }}</el-button>
...
<script>
import { getByPageTask, createTask, getAllDevops, deleteTask } from '@/api/devops.js'
export default {
  data() {
    return {
      listLoading: true,
  ...
  methods: {
    ...
    create() {
      // 是否新建？
      this.$confirm(this.$t('devopstools.createConfirm'), this.$t('common.prompt'), {
        // 确认
        ...
        this.listLoading = true
        createTask(this.creatFrom).then(res => {
          if (res.code === 0) {
            // 重置翻页组件数据
            ...
            this.$message({
              type: 'success',
              message: this.$t('devopstools.createSuccess'),
              duration: 1000
            })
          }
        }).finally(() => {
    ...
    // 打开文件上传页面
    showDialogVisible() {
      this.creatFrom.taskName = ''
      this.creatFrom.devopsId = ''
