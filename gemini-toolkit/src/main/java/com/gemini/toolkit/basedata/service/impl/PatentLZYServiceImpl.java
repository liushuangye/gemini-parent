...
import com.gemini.toolkit.basedata.service.PatentLZYService;
import com.gemini.toolkit.common.utils.*;
...
@Slf4j
@Service
public class PatentLZYServiceImpl extends AbsDataImport implements PatentLZYService {
    @Override
    ...
    public void afterImportData(HSSFWorkbook wookbook, String templateType, String dbType) {
        ...
        // 获取要特殊处理得sheet名
        // 组织人员
        String sheetName = CommonConsts.PATENT_LZYSHEET;
            ...
            for (int i = 16; i < sheet.getLastRowNum() + 1; i++) {
                ...
                if (CommonConsts.INSERT.equals(updateFlg)) {
                    ...
                    for (int m = 2; m < lastCellNum - 1; m++) {
                        ...
                        if (ach.contains("专利权人")) {
                            String company = null;
                            List<String> staffIds = customizeMapper.getStaffIds(value);
                            if (staffIds != null && staffIds.size() == 1) {
                                List<Map<String, Object>> results = customizeMapper.getResults("SELECT organize_name from t_organize where organize_id = " +
                                        "(SELECT organize_id from t_organize_staff where staff_id = '" + staffIds.get(0) + "')");
                                company = (String) results.get(0).get("organize_name");
                            } else {
                                company = value;
                            }
                            ...
                            if (company.contains("附属")) {
                                unit.setValue("10");
                            } else {
                                unit.setValue("20");
                                SaveOrUpdateDto unit2 = new SaveOrUpdateDto();
                                unit2.setValue(company);
